...
@patch('requests.post', new=functools.partial(MyResponse.post, False))
def test_extract(create_buildhub_data):
    data = buildhub.get_raw()
    data, buildids, buildids_per_prod = buildhub.extract(data)
    data = {'data': data,
            'buildids': buildids,
            'buildids_per_prod': buildids_per_prod}
    x = get_or_create('tests/data/buildhub/extract.json', data)
...
@patch('requests.post', new=functools.partial(MyResponse.post, False))
def test_get_last_versions(create_buildhub_data):
    data = buildhub.get_raw()
    data, _, _ = buildhub.extract(data)
    buildhub.get_last_versions(data)
    x = get_or_create('tests/data/buildhub/last_versions.json', data)

    assert x == data


@patch('requests.post', new=functools.partial(MyResponse.post, False))
def test_add_unicity(create_buildhub_data):
    data = buildhub.get_raw()
    data, buildids, buildids_per_prod = buildhub.extract(data)
    buildhub.add_unicity_info(data, buildids, buildids_per_prod)
    x = get_or_create('tests/data/buildhub/unicity.json', data)
...
def test_get(create_buildhub_data):
    data = buildhub.get(bid_as_date=False)
    x = get_or_create('tests/data/buildhub/get.json', data)
